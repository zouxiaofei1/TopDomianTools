...
"Down","Up","In","On","Back","Front","Teacher" };//滚动显示的字符
...
public:
	void InitClass(HINSTANCE HInstance)//新Class使用之前最好Init一下
	{
		...
		CoverCheck = 0;
		//默认宋体
		DefFont = CreateFontW(max((int)(16 * DPI), 10), max((int)(8 * DPI), 5), 0, 0, FW_THIN, FALSE, FALSE, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, PROOF_QUALITY, DEFAULT_PITCH | FF_SWISS, L"宋体");
	}
	...
	void SetStr(LPCWSTR Str, LPCWSTR ID)//通过ID设置字符串
	{
		...
		str[HashTemp] = new wchar_t[mywcslen(Str) + 1];//申请相应的内存空间
		mywcscpy(str[HashTemp], Str);//复制新的
	}

	__forceinline void CreateString(LPCWSTR Str, LPCWSTR ID)//创建新字符串
	{//注意:应仅在使用常量初始化时使用本函数,用变量初始化是建议CurString++再用SetStr
		++CurString;
		if (Str != NULL)//默认仅复制指针(危险!)
#ifdef _DEBUG
			str[Hash(ID)] = new wchar_t[mywcslen(Str) + 1],
			...
			colorok:
				...
				{//正在下载
					...
					else
					{
						...
						{
							wchar_t TempStr[20] = {0}, TempNumber[MAX_NUM] = { 0 };//正在下载 (已下载个数)/(总数)
	...
	void EditPaste(int cur)//将剪贴板中文字粘贴到Edit中的函数
	{
		...
		{//打开剪切板
			...
			{//如果只有单光标选中
				...
				Edit[cur].Pos1 += (int)mywcslen(ClipBoardTemp); delete[] ClipBoardTemp;
...
#pragma warning(disable:28183)
	...
	{//这些函数都是代码长而原理简单，因此没有注释
		...
		__except (EXCEPTION_EXECUTE_HANDLER) { InfoBox(L"unk"); return FALSE; }
	}
	...
	{//将语言文件中读取到的一行设置到Class中
		...
		{
			GETLAN gl = { 0 };
			wchar_t* pos = mywcsstr(ReadTmp, L"=");
			if (pos == 0)return;
			*pos = 0; gl.begin = pos + 1;
			wchar_t* space = mywcsstr(ReadTmp, L" ");
			...
			if (type == 1)//button
			{
				...
				if (gl.str2 != NULL)
				{
					Button[cur].Exp = new wchar_t[mywcslen(gl.str2) + 1];
...
#pragma warning(default:28183)
	...
	HFONT DefFont;//默认的字体
	int Msv = 0;//鼠标移出检测变量
	int CurString=0, CurButton = 0, CurFrame = 0, CurCheck = 0, CurLine = 0, CurText = 0, CurEdit = 0, CurArea = 0;//各种控件的数量
	float DPI = 1;//缩放大小
	int CoverButton=0, CoverCheck=0, CoverEdit=0, CoverArea=0;//当前被鼠标覆盖的东西
	BOOL Obredraw = TRUE;//是否启用ObjectRedraw技术 - 默认开启
	BOOL ButtonEffect = FALSE;//是否开启渐变色
	int CurWnd=1;//当前的页面
	int Press=FALSE;//鼠标左键是否按下
	...
	RECT es[MAX_ES];//清理列表
	HDC hdc=0, tdc=0;//缓存 and 真实dc
	HBITMAP EditBitmap=0;//Edit专用缓存bitmap
	HBITMAP Bitmap=0;//缓存窗口bitmap
	int Width=0, Height=0;//窗口的宽和高
	HWND hWnd=0;//Class绘制的窗体的hwnd
	HINSTANCE hInstance=0;//程序的hInst
...
public://这些函数中有些参数没有用到，会导致大量警告.
	int curi=0;/*推送下载进度信息的按钮编号*/ int factmax = 0;//手动设置被下载文件的真实大小
...
#pragma warning(default:4312)
				...
				{
					wchar_t cmdline[MAX_PATH] = {0}, PIDStr[MAX_NUM] = { 0 };
...
void UpdateInfo()//修改"关于"界面信息的函数.
{
	...
	mywcscpy(tmp2, Main.GetStr(L"TIP")); CheckIP(tmp2);//ip地址
...
{//刷新主界面上"已捕捉 X 个窗口"这个字符串
	wchar_t tmpstr[MAX_PATH] = {0}, tmpnum[MAX_NUM] = { 0 };
...
{//这个函数在用在其他工程时不能直接照抄，因为涉及到不同窗体的问题
	__try {//为了防止直接崩溃这边都用_try包住了
		...
		int type = 0; DWORD NumberOfBytesRead;const bool ANSI = mywcsstr(name, L"ANSI");
		...
		Main.EnableButton(BUT_MORE, !(BOOL)mywcsstr(name, L"English"));//几个文字会改变的按钮
		if (mywcsstr(name, L"English"))Main.Area[4].Left = 230, Main.Area[4].Width = 110; else Main.Area[4].Left = 176, Main.Area[4].Width = 85;
...
void CALLBACK TimerProc(HWND hWnd, UINT nMsg, UINT nTimerid, DWORD dwTime)//主计时器
{
	...
	case TIMER_CATCHWINDOW://延时捕捉窗口
	{
		if (--CatchWndTimerLeft >= 0)
		{
			...
			wchar_t TempString[MAX_STR] = {0}, TempNumber[MAX_NUM] = { 0 };
	...
	case TIMER_COPYLEFT://copyleft
	{
		...
		wchar_t CopyLeftstr[MAX_STR] = L"Copy", Tempstr[10] = {0};//设置文字滚动
		MultiByteToWideChar(CP_ACP, 0, EasterEggStr[EasterEggState % 11], -1, Tempstr, 10);
		mywcscat(CopyLeftstr, Tempstr);
		mywcscat(CopyLeftstr, Main.GetStr(L"Tleft"));
		Main.SetStr(CopyLeftstr, L"_Tleft");
	darrk:
		col = ((myrand() % 256) << 16) + ((myrand() % 256) << 8) + ((myrand() % 256));//随机一个颜色出来
...
{//自动将获得的密码用MessageBox显示
	...
	if (MessageBox(Main.hWnd, TempStr, Main.GetStr(L"Info"), MB_ICONINFORMATION | MB_OKCANCEL) == IDOK)
	{
		...
		char* ClipBoardStr = new char[len];
...
BOOL RunCmdLine(LPWSTR str)//解析启动时的命令行并执行
{
	...
	//setlocale(LC_ALL, "chs");//这一行语句会增加程序体积约30k，谨慎使用
	slient = TRUE;
	...
	if (mywcsstr(str, L"-clear") != NULL) { wchar_t tmp[MAX_STR] = { 0 }; AutoChangePassword(tmp, 1); goto okreturn; }
	...
	{//因为changepasswd包括change这个字符串
		wchar_t tmp[MAX_PATH] = { 0 }, * tmp1 = mywcsstr(str, L"-changewithpasswd");
		if (!Findquotations(tmp1, tmp))goto error;
	...
	{
		wchar_t tmp[MAX_PATH] = { 0 }, * tmp1 = mywcsstr(str, L"-channel");
		if (!Findquotations(tmp1, tmp))goto error;
	...
	{
		wchar_t tmp[MAX_PATH] = { 0 }, * tmp1 = mywcsstr(str, L"-change");
		if (!Findquotations(tmp1, tmp))goto error;
