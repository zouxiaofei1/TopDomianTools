...
wchar_t Path[MY_MAX_PATH], Name[MY_MAX_PATH];//程序路径 and 路径+程序名 (Path最后有"\")
...
HPEN YellowPen, BlackPen, WhitePen, TitlePen, CheckGreenPen, NormalGreyPen, NormalBluePen, DBluePen, LBluePen, BSODPen;//笔
...
wchar_t TDPath[MY_MAX_PATH], TDName[MY_MAX_PATH] = L"StudentMain.exe";
bool TDsearched = false;
bool SethcState = true;//System32目录下sethc是否存在
bool SethcInstallState = false;//Sethc方案状态
bool HookState = false;//全局键盘钩子方案状态
BOOL FirstFlag;//是否在这台电脑上第一次运行?(根据sethc是否被备份检测)
...
public:
	...
	{								//  （不Init也行）
		...
		CurWnd = 1;//清"零"
		CoverBotton = -1;//ps:可以在InitClass之前设定DPI
		CoverCheck = 0;
		//默认宋体
	...
	{
		if (str[Hash(ID)] != 0)str[Hash(ID)] = 0;//删除当前ID中原有的字符串
		str[Hash(ID)] = new wchar_t[wcslen(Str) + 1];//申请相应的内存空间
	...
	void Redraw() 
	{
		while (!rs.empty())Main.rs.pop();
		while (!es.empty())Main.es.pop(); 
		InvalidateRect(hWnd, nullptr, false); 
		UpdateWindow(hWnd); }//添加要刷新的控件-、
...
HWND tdhw, tdhw2;
BOOL CALLBACK EnumChildwnd(HWND hwnd, LPARAM lParam)//查找被监视窗口的枚举函数.
{
...
BOOL CALLBACK EnumBroadcastwnd(HWND hwnd, LPARAM lParam)//查找被监视窗口的枚举函数.
{
	UNREFERENCED_PARAMETER(lParam);
	//ULONG nProcessID;
	LONG A;//遍历所有窗口
	...
	if (wcsstr(title, L"屏幕广播") != 0)
	{
		...
		HWND t = CreateWindow(ScreenWindow, L"You can't see me.", WS_POPUP, 0, 0, 100, 100, nullptr, nullptr, hInst, nullptr);
		SetParent(hwnd, t);
		tdhw2 = hwnd;
...
void KillTop()//杀掉置顶窗口
{
	...
	if (!EnumWindows(EnumTopWnd, NULL))error();
}
...
{
	Main.Frame[7].rgb = RGB(255, 180, 10);
	if (!Admin)//t = 不可用 ; ok = 可用 ; rec = 推荐 ; nrec = 不推荐
	{
		int t[] = { 1,6,8 }, ok[] = { 2, 3 },/* rec[] = { 2 }, */nrec[] = { 4 }, i;
		for (i = 0; i < 3; ++i)Main.Frame[t[i]].rgb = COLOR_RED, wcscat_s(Main.Frame[t[i]].Name, Main.GetStr(L"Useless"));
...
{//这个函数在用在其他工程时不能直接照抄，因为涉及到不同窗体的问题
	__try {//为了防止直接崩溃这边都用_try包住了
		...
		while (!feof(fp))
		{
			...
			if (ReadTmp[0] != '<')
			{
				if (Mainf)DispatchLanguage(ReadTmp, type, &Main);//分配(各个)窗体的语言
			}
		}
		fclose(fp);
		...
		Main.Button[Main.GetNumbyID(L"more.txt")].Enabled = !(bool)wcsstr(name, L"English");//几个文字会改变的按钮
...
BOOL SetupSethc()//安装sethc.
{
	...
	wcscpy_s(mySethcPath, Path);
	wcscat_s(mySethcPath, L"sethc.exe");
...
{//自动使用驱动 or 应用层删除一个文件.
	if (Main.Check[CHK_T_A_].Value == 1)
	{
		...
		wcscpy_s(DriverPath, L"\\??\\");/*这里路径前要加上\??\			*/
...
{//控制伪装工具条(新)窗体的伸出 or 缩回的进程
	...
	{
		Sleep(10);//伸缩动画
		SetWindowPos(FakeWnd, 0, rc.left, rc.top + i * 2 * offset, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOREDRAW);
	}
...
{//点击窗体左上角的logo就可以触发(隐藏功能)
	...
	if (((int)((byte)cc.rgbResult) + (int)((byte)(cc.rgbResult >> 8)) + (int)((byte)(cc.rgbResult >> 16))) <= 384)
		Main.Text[25].rgb = RGB(255, 255, 255); else Main.Text[25].rgb = 0;//颜色较浅时字体为黑色，反之亦然
	return cc.rgbResult;
}
BOOL CALLBACK CatchThread(HWND hwnd, LPARAM lParam)//捕捉窗口.
{
	...
	{//如果是顶级窗口(没有父窗口的窗口)
		...
		if (Eatpid[nProcessID] == true && IsWindowVisible(hwnd))
		{
			RECT rc, rc2;
			GetWindowRect(hwnd, &rc);
			if (rc.bottom<0 || rc.right<0 || rc.left> GetSystemMetrics(SM_CXSCREEN) || rc.top> GetSystemMetrics(SM_CYSCREEN))return 1;//不在屏幕范围内的窗口将不被监视
			if (rc.right - rc.left < 15 || rc.bottom - rc.top < 15)return 1;//过小的窗口将不被监视
...
void ReturnWindows()//归还窗口.
{
	if (tdhw != 0 && tdhw2 != 0)
	{
		KillTimer(Main.hWnd, 12);
		//SetParent(tdhw2, NULL);
		SetParent(tdhw, NULL);
...
bool RefreshTDstate()//刷新极域的状态
{
	...
	{//寻找极域进程
		...
		{//极域被启动了
			...
			Main.Text[10].rgb = RGB(0, 255, 0);//设置一些按钮的状态，然后重绘
	...
	Main.Text[10].rgb = RGB(255, 0, 0);//设置一些按钮的状态，然后重绘
...
void CALLBACK TimerProc(HWND hWnd, UINT nMsg, UINT nTimerid, DWORD dwTime)//主计时器
{
	...
	case 1://连续结束进程
	{
		if (Main.Check[CHK_REKILL].Value == 1)
			if (!KillProcess(Main.Edit[Main.GetNumByIDe(L"E_TDname")].str))
			{
				...
				KillTimer(hWnd, 1);//结束进程 全部 失败则退出
	...
	case 2://延时捕捉窗口
	{
		int cur = Main.GetNumbyID(L"CatchW");
		timerleft--;
		if (timerleft >= 0)
		{
			Main.Button[cur].Enabled = false;
			...
			wcscpy_s(Main.Button[cur].Name, tmp);//拼接按钮中"剩余XX秒"的文字
			Main.Readd(2, cur);
			Main.Redraw(Main.GetRECT(cur));
			if (timerleft == 0)CatchWindows();
		}
		if (timerleft == -1)Main.Button[cur].Enabled = true, wcscpy_s(Main.Button[cur].Name, Main.GetStr(L"back")), Main.Readd(2, cur), Main.Redraw(Main.GetRECT(cur));
		break;
	}
	case 3://copyleft
	{
	...
	case 4://定时创建exp & 其他功能
	{
		...
		{//slow
			Main.EditUnHotKey();
			Main.CoverBotton = Main.CoverEdit = Main.CoverEdit = 0;
	...
	case 12:
	{
		...
		if (!SetWindowPos(tdhw, 0, 0, 0, rc.right - rc.left, rc.bottom - rc.top, SWP_NOREDRAW))KillTimer(Main.hWnd, 12);
		//ShowWindow(tdhw2, SW_SHOW);
		break;
	}
	}
}
...
{//(仅用于创建线程)
	(pM);
	int num = Main.GetNumbyID(L"re-TD");
	if (Main.Button[num].Enabled == false)return 0;
	if (!TDsearched)
	{
		TDsearched = true;
		if (!slient)
		{
			Main.Button[num].Enabled = false;
			Main.Readd(2, num);
...
void AutoChangeChannel(int ChannelID)
{
	HKEY hKey;
	LONG ret;
	if (Bit != 64)
...
bool RunCmdLine(LPWSTR str)//解析启动时的命令行并执行
{
	...
	{
		Main.Check[CHK_TOP].Value = Main.Check[CHK_EFFECT].Value = 1;
		TOP = TRUE;
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)//初始化
{
	...
	Main.CreateText(195, 285, 1, L"Tctrl+b", RGB(255, 100, 0));
	Main.CreateLine(185, 315, 565, 315, 1, 0);

	Main.CreateText(195, 330, 1, L"Processnam", 0x7B7672);//想要捕捉的进程名
	Main.CreateText(463, 330, 1, L"Delay", 0x7B7672);//延迟时间
	Main.CreateText(510, 370, 1, L"second", 0x7B7672);//s
	...
	Main.CreateText(195, 480, 1, L"_Eat", 0xDA8B01);
	...
	Main.CreateText(365, 300, 2, L"Tcp1", RGB(50, 50, 50));
	Main.CreateText(365, 325, 2, L"Tcp2", RGB(255, 100, 0));
	...
	Main.CreateText(250, 405, 2, L"TcmdNO", RGB(255, 0, 0));
	...
	Main.CreateButton(345, 385, 115, 55, 2, L"reserved", L"windows.ex");
	Main.CreateButton(480, 385, 115, 55, 2, L"防止教师关机", L"ANTI-");
	Main.CreateButton(345, 460, 115, 55, 2, L"显示于安全桌面", L"desktop");
	Main.CreateButton(480, 460, 115, 55, 2, L"自动防控制", L"auto-5");
	...
	Main.CreateButton(470, 470, 105, 50, 5, L"清理并退出", L"clearup");
	...
	Main.CreateButtonEx(Main.CurButton, 530, 10, 60, 30, 0, L"×", \
		...
		Main.DefFont, 1, RGB(255, 255, 255), L"Close");//关闭 按钮
	...
	case WM_LBUTTONUP://抬起鼠标左键时
	{
		...
		for (int i = 1; i <= Main.CurArea; ++i)//Area 响应区域
		{
			if (Main.InsideArea(i, point) == TRUE && (Main.Area[i].Page == 0 || Main.Area[i].Page == Main.CurWnd))
			{
				...
				case 4:
					if (RunWithAdmin(Name))MyExitProcess();//以管理员权限重启
					break;
				}
			}
		}
		...
		BUTTON_IN(x, L"P2") { Main.SetPage(2); EasterEgg(false); ShowWindow(FileList, SW_HIDE); SetTimer(Main.hWnd, 8, 250, (TIMERPROC)TimerProc); RefreshTDstate();   break; }
		BUTTON_IN(x, L"P3") { Main.SetPage(3); EasterEgg(false); ShowWindow(FileList, SW_HIDE); KillTimer(Main.hWnd, 8); break; }
		BUTTON_IN(x, L"P4") { if (!InfoChecked) UpdateInfo(), ReleaseRes(L"C:\\SAtemp\\1.JPG", IMG_ZXF, L"JPG"), InfoChecked = true; Main.SetPage(4); ShowWindow(FileList, SW_HIDE); KillTimer(Main.hWnd, 8); break; }
		BUTTON_IN(x, L"P5")
		{
			if (FileList == NULL)
			{
			...
			KillTimer(Main.hWnd, 8); SearchLanguageFiles(); break;
		}//切换到第五页时搜索语言文件
		...
		BUTTON_IN(x, L"hook")//安装hook,又名"全局键盘钩子"
		{
		...
		BUTTON_IN(x, L"SwitchD") { CreateThread(0, 0, SDThread, 0, 0, 0); break; }//切换桌面
		BUTTON_IN(x, L"CatchW")
		{
			AutoCreateCatchWnd();
			timerleft = _wtoi(Main.Edit[Main.GetNumByIDe(L"E_Delay")].str);
			Main.SetStr(Main.Button[Main.CoverBotton].Name, L"back");
			if (timerleft < 1)timerleft = 1;
			SetTimer(Main.hWnd, 2, 1000, (TIMERPROC)TimerProc);
			break;
		}
		BUTTON_IN(x, L"CopyW")
		{
			...
			SetTimer(Main.hWnd, 6, 33, (TIMERPROC)TimerProc);//注意:这里的timerproc是绑定再Main上的
		...
		BUTTON_IN(x, L"ReturnW")
		{
			ReturnWindows(); break;
		}
		BUTTON_IN(x, L"ApplyCh") { AutoChangeChannel(_wtoi(Main.Edit[Main.GetNumByIDe(L"E_ApplyCh")].str)); break; }//改变频道
		BUTTON_IN(x, L"ClearPass") { wchar_t tmp[10] = { 0 }; AutoChangePassword(tmp, 1);  break; }//A U T O
		...
		if (x == Hash(L"CP1") || x == Hash(L"CP2"))
		{
			...
			str[32] = str[33] = 0;
			if (x == Hash(L"CP1"))AutoChangePassword(str, 1); else AutoChangePassword(str, 2); break;
		}
		BUTTON_IN(x, L"kill-TD") { AutoTerminateTD(); break; }
		BUTTON_IN(x, L"re-TD") { CreateThread(NULL, 0, ReopenThread, NULL, 0, NULL);  break; }
		BUTTON_IN(x, L"windows.ex")
		{
			...
			{
				SetTimer(Main.hWnd, 12, 100, (TIMERPROC)TimerProc);
				TOP = FALSE;
				KillTimer(hWnd, 10);
		...
		{
			CreateThread(NULL, 0, DeleteThread, 0, 0, NULL);
			break;
		}
		...
		BUTTON_IN(x, L"hidest") { HideState = -1; ShowWindow(Main.hWnd, SW_HIDE); break; }//永久隐藏
		...
		BUTTON_IN(x, L"Close") { MyExitProcess(); }//"关闭"按钮
			...
			if (Main.Check[i].Page == 0 || Main.Check[i].Page == Main.CurWnd)
			{
				...
				if (result != 0)
				{
					...
					{//未选中->选中
						...
						case CHK_FTOLD:case CHK_FTNEW: {//伪装工具条
							...
							if (FF == TRUE)//注册类
							{
								...
								SetClassLong(FakeWnd, GCL_STYLE, GetClassLong(FakeWnd, GCL_STYLE) | CS_DROPSHADOW);
							}//没有任务栏图标的"工具栏窗口"
							if (i == CHK_FTOLD)
							{
							...
							else
							{
								...
								FakeTimer = GetTickCount();
								SetTimer(FakeWnd, 11, 100, (TIMERPROC)TimerProc);
							}
						...
						case CHK_TOOLBAR: {
							...
							break; }//CHK_TDINVD 4
						case CHK_RETD: {RegisterHotKey(Main.hWnd, 14, MOD_CONTROL, 'Y'); break; }
						case CHK_T_A_:
						{//加载驱动删除文件
						...
						case CHK_BSOD: {RegisterHotKey(Main.hWnd, 4, MOD_CONTROL, 'R'); break; }//蓝屏
						case CHK_SHUTD: {RegisterHotKey(Main.hWnd, 5, MOD_CONTROL, 'T'); break; }//重启
						case CHK_SCSHOT: {//截图伪装
							...
							break; }
						case CHK_REKILL: {SetTimer(hWnd, 1, 1500, (TIMERPROC)TimerProc); break; }//连续结束进程	//CHK_FMACH 12
						case CHK_NOHOOK: {SetTimer(hWnd, 7, 100, (TIMERPROC)TimerProc); break; }//禁止键盘（鼠标）钩子
					...
					{//选中 -> 未选中
						switch (i)
						{//关闭"伪装工具条"
						case CHK_FTOLD: case CHK_FTNEW: {ShowWindow(FakeWnd, SW_HIDE); KillTimer(FakeWnd, 11); break; }
						case CHK_TOOLBAR: {
							NOTIFYICONDATA tnd;
							tnd.cbSize = sizeof(NOTIFYICONDATA);
							tnd.hWnd = Main.hWnd;
							...
							Shell_NotifyIcon(NIM_DELETE, &tnd);
							break; }//CHK_TDINVD 4
						case CHK_RETD: {UnregisterHotKey(Main.hWnd, 14); }//CHK_T_A_ 6
						case CHK_TOP: {
							TOP = FALSE;
							KillTimer(hWnd, 10);
							if (CatchWnd != NULL)SetWindowPos(CatchWnd, HWND_NOTOPMOST, 0, 0, 0, 0, 1 | 2);//取消置顶
						...
						case CHK_REKILL: {KillTimer(hWnd, 1); break; }//CHK_FMACH 12
						case CHK_NOHOOK: {KillTimer(hWnd, 7); break; }
