...
wchar_t szVDesk[] = L"GUIdesk", fBSODdesk[] = L"GUIBSOD";//虚拟桌面 & 蓝屏伪装窗口桌面的名称
...
public:
	...
	void InfoBox(LPCWSTR Str)//全自动的MessageBox
	{
		...
		{
			if (GUIStrExist)WriteConsole(hdlWrite, GetStr(Str), (DWORD)wcslen(GetStr(Str)),nullptr, nullptr); else WriteConsole(hdlWrite, Str, (DWORD)wcslen(Str), nullptr, nullptr);
		...
		{//发现是一个文件
			...
			if (type == 1)
				...
				{//查找并运行studentmain
					...
					if (Main.Check[CHK_TDINVD].Value == 1)RunEXE(szFile, NULL, &si); else RunEXE(szFile, NULL, nullptr);
				}
...
void AutoDelete(wchar_t* tmp, bool sli)//自动删除文件
{
	...
	{//不是文件也不是文件夹？是否强制删除？
		if (!sli)
		{
			...
			wcscat_s(tmp2, tmp);
			wcscat_s(tmp2, L"\"");
			wcscat_s(tmp2, Main.GetStr(L"TINotF"));
			if (MessageBox(Main.hWnd, tmp2, Main.GetStr(L"Info"), MB_YESNO | MB_ICONQUESTION) == 6)goto a;
		}
		return;
	}
a:
	...
	if (FileType & FILE_ATTRIBUTE_DIRECTORY)
	{
		wchar_t tmp3[301];
		wcscpy_s(tmp3, tmp);
		SearchTool(tmp3, 3);//是文件夹
		MyRemoveDirectory(tmp3);//shfileoperation需要对字符串操作
	}
	else
...
{//控制伪装工具条(新)窗体的伸出 or 缩回的进程
	...
	{
		Sleep(1);//伸缩动画
		SetWindowPos(FakeWnd, 0, rc.left, rc.top + i * offset, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOREDRAW);
	}
...
void CALLBACK TimerProc(HWND hWnd, UINT nMsg, UINT nTimerid, DWORD dwTime)//主计时器
{
	...
	case 1://连续结束进程
	{
		if (Main.Check[CHK_REKILL].Value == 1)KillProcess(Main.Edit[Main.GetNumByIDe(L"E_TDname")].str);
		break;
	}
	case 2://延时捕捉窗口
	{
	...
	case 3://copyleft
	{
		...
		RECT rc{ (int)(170 * Main.DPI),(int)(417 * Main.DPI),(int)(420 * Main.DPI),(int)(610 * Main.DPI) };
		Main.Redraw();
		break;
	}
	case 4://定时创建exp & 其他功能
	...
	case 10://循环置顶(低画质时启用)
		...
		break;
	case 11:
		if (GetTickCount() - FakeTimer > 2500)
		{
			RECT rc;
			GetWindowRect(FakeWnd, &rc);
...
{
	if (slient)
		WriteConsole(hdlWrite, str, (DWORD)wcslen(str), nullptr, nullptr);
	else
	{
		wchar_t Tmp[300];
...
bool RunCmdLine(LPWSTR str)//解析启动时的命令行并执行
{
	...
	{//因为changewithpasswd包括change这个字符串
		...
		if (!Findquotations(tmp1, tmp))MyExitProcess();
