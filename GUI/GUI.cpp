	...
	void DrawButtons(int cur)//绘制按钮
	{
		...
		begin:
			if (Button[i].Page == CurWnd || Button[i].Page == 0)
			{
				...
				if (Button[i].Percent != 0 && Button[i].Percent != 100 && Button[i].DownTot == 0)//绘制渐变色
				{
				...
				if (CoverButton == i && Button[i].DownTot == 0)//没有禁用&渐变色时使用默认颜色
					if (Press) {
...
#pragma warning(disable:26495)//禁用"始终初始化成员变量"的警告
	...
	void EditPaste(int cur)//将剪贴板中文字粘贴到Edit中的函数
	{
		...
		{//打开剪切板
			...
			const size_t len = strlen(buffer), len2 = Edit[cur].strLength + 1;
	...
	void LeftButtonDown()//鼠标左键按下
	{
		...
		{//鼠标点在被激活的Edit外面时将CoverEdit设为0;
			...
			const int TempCoverEdit = CoverEdit;
			CoverEdit = 0;
			EditRedraw(TempCoverEdit);
			EditUnHotKey();//取消热键
		}
		if (EditPrv != 0)
		{//重置之前被点击的Edit的颜色
...
public:
	int curi;/*推送下载进度信息的按钮编号*/ int factmax;//被下载文件的真实大小(可不填)
...
void SwitchLanguage(LPWSTR FilePath)//切换语言的函数
{
	...
	while (1)
	{
		...
		*point2 = 0;
		...
		if (point1[0] != L'<')
		{
			/*if (Mainf)*/Main.DispatchLanguage(point1, type);//分配(各个)窗体的信息
		}
		point1 = point2 + 1;
	}
...
{//UT开启的时候浮在最上面，看不到弹出的MessageBox,因此Class里的InfoBox暂时不能使用
	...
	--Main.CurFrame;//UT开启时无法显示List,所以我们在这里显示一个占位用的Frame
...
{//这是一个不稳定的函数
	...
	while (1)
	{
		...
		ReleaseDC(Main.hWnd, Main.tdc); Sleep(10);
	}
	return 0;
}
...
DWORD WINAPI InitThread(LPVOID pM)//创建各种控件(线程)
{
	...
	Main.CreateButtonEx(1, 1, 50, 139, 64, 0, L"主要功能", LightestGreyBrush, LightBlueBrush, LighterBlueBrush, WhitePen, LightBluePen, LighterBluePen, 0, TRUE, 0, 0, L"P1");//切换页面按钮
	Main.CreateButtonEx(2, 1, 115, 139, 64, 0, L"极域工具箱", LightestGreyBrush, LightBlueBrush, LighterBlueBrush, WhitePen, LightBluePen, LighterBluePen, 0, TRUE, 0, 0, L"P2");//
	Main.CreateButtonEx(3, 1, 180, 139, 64, 0, L"其他工具", LightestGreyBrush, LightBlueBrush, LighterBlueBrush, WhitePen, LightBluePen, LighterBluePen, 0, TRUE, 0, 0, L"P3");
	Main.CreateButtonEx(4, 1, 245, 139, 64, 0, L"关于", LightestGreyBrush, LightBlueBrush, LighterBlueBrush, WhitePen, LightBluePen, LighterBluePen, 0, TRUE, 0, 0, L"P4");
	Main.CreateButtonEx(5, 1, 310, 139, 64, 0, L"设置", LightestGreyBrush, LightBlueBrush, LighterBlueBrush, WhitePen, LightBluePen, LighterBluePen, 0, TRUE, 0, 0, L"P5");
	Main.CreateButtonEx(6, 1, 375, 139, 173, 0, L"一键安装", LighterGreyBrush, LightBlueBrush, LighterBlueBrush, WhitePen, LightBluePen, LighterBluePen, 0, TRUE, 0, 0, L"QS");
	Main.CurButton = 6;
	Main.CreateLine(140, 51, 498, LINE_Y, 0, COLOR_NORMAL_GREY);
	Main.CreateLine(0, 51, 498, LINE_Y, 0, COLOR_NORMAL_GREY);//切换页面按钮边上的线
