...
public:
	...
	{//						(由于某历史原因，缩放大小的变量被我命名成了DPI)
		...
		DefFont = CreateFontW(max((int)(16 * DPI), 8), max((int)(8 * DPI), 4), 0, 0, FW_THIN, FALSE, FALSE, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, _T("宋体"));
...
void CALLBACK TimerProc(HWND hWnd, UINT nMsg, UINT nTimerid, DWORD dwTime)//主计时器
{
	...
	{//用于获得鼠标信息的计时器
		...
		BOOL LBnew = KEY_DOWN(VK_LBUTTON), CSinside = FALSE;//为了避免这一现象，这里将Main变成了正常置顶，刚好处于UT正下方来"阻挡"住点击信号。
...
BOOL RunCmdLine(LPWSTR str)//解析启动时的命令行并执行
{
	...
	if (wcsstr(str, L"-enablebut") != NULL) { SetTimer(0, TIMER_ONEKEY, 1500, (TIMERPROC)TimerProc); Main.InfoBox(L"suc"); goto noreturn; }//一键安装(EnableButton)
	if (wcsstr(str, L"-killtop") != NULL) { if (KillFullScreen()) Main.InfoBox(L"suc"); goto okreturn; } //自动结束置顶进程
	if (wcsstr(str, L"-unsethc") != NULL) { UninstallSethc(); goto okreturn; }//卸载sethc
	if (wcsstr(str, L"-vp") != NULL) { AutoViewPass(); goto okreturn; }//显示密码
	...
	if (wcsstr(str, L"-changepasswd") != NULL)//前后二者顺序不能调换
